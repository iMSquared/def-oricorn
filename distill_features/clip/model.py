...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward_k(self, x: torch.Tensor):
        """
        Forward function for computing the key features for dense prediction (i.e., features for every image patch).
        """
        # Get the weights and biases for the value projection, multihead attention uses 3 * embed_dim for the input projection
        k_in_proj_weight = self.attn.in_proj_weight[self.attn.embed_dim:-self.attn.embed_dim]
        k_in_proj_bias = self.attn.in_proj_bias[self.attn.embed_dim:-self.attn.embed_dim]

        k_in = F.linear(self.ln_1(x), k_in_proj_weight, k_in_proj_bias)
        k_out = F.linear(k_in, self.attn.out_proj.weight, self.attn.out_proj.bias)
