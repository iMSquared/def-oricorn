    ...
    def __call__(self, imgs, cam_posquat, cam_intrinsic, train=False):
        ...
        if self.args.spatial_pe:
            ...
            if self.args.dif_model_version == 4:
                ...
                # patch_size = [8,14]
                # img_feat_flat = einops.rearrange(img_feat_structs.img_feat + img_feat_structs.spatial_PE, '... v (r1 i) (r2 j) c -> ... (v r1 r2) (i j c)', r1=patch_size[0], r2=patch_size[1])
                # img_feat_flat = nn.Dense(img_base_dim)(img_feat_flat)
...
class SpatialPE(nn.Module):
    @nn.compact
    def __call__(self, img_feat:structs.ImgFeatures):
    ...
    def __call__(self, z, context, img_feat_flat=None, train=False):
        ...
        if not self.local_only:
            if self.args.dif_model_version in [6]:
                ...
                # global mixing -> conditioning (cross attention)
                z_gb = einops.rearrange(z, '... s c f -> ... (s c) f')
...
class DenoisingModelV4(nn.Module):
    ...
    def __call__(self, objects_ptcl:cxutil.LatentObjects, con_feat:structs.ImgFeatures, time, batch_condition_mask=None, confidence_out=False, train=False):
        ...
        pos_emb = nn.Dense(self.args.base_dim//2)(c_in[...,None]*objects_ptcl.pos)
        pos_emb = jnp.sin(pos_emb)
        pos_emb = nn.Dense(self.args.base_dim//2)(pos_emb)
        
        if self.args.use_p:
            cen_emb = nn.Dense(self.args.base_dim//2)(c_in[...,None,None]*objects_ptcl.dc_centers_tf)
        ...
        if self.args.dif_model_version in [1,4,6]:
            # PATR
            if self.args.use_p:
                selected_img_fts, cam_fts = extract_pixel_features(objects_ptcl.dc_centers_tf*c_in_, con_feat)
            else:
                selected_img_fts, cam_fts = extract_pixel_features(objects_ptcl.pos[...,None,:]*c_in_, con_feat)
            
            if self.args.spatial_pe:
                z_con = selected_img_fts # (B NO ND NC d)
            else:
                for _ in range(2):
            ...
            if batch_condition_mask is not None:
                z_con = jnp.where(batch_condition_mask[...,None,None,None], z_con, 0)
            if self.args.dif_model_version in [6]:
                context = embeddings
                if self.args.use_p:
                    img_feat_flat = jnp.c_[z_con, cen_emb]
                else:
                    img_feat_flat = z_con
            else:
                if self.args.use_p:
                    if self.args.one_step_test == 3:
                        context = jnp.c_[z_con, cen_emb]
                    else:
                        context = jnp.c_[z_con, cen_emb, jnp.broadcast_to(embeddings, jnp.broadcast_shapes(z_con.shape, embeddings.shape))]
                else:
                    if self.args.one_step_test == 3:
                        context = jnp.c_[z_con, pos_emb[...,None,:]]
                    else:
                        context = jnp.c_[z_con, pos_emb[...,None,:], jnp.broadcast_to(embeddings, jnp.broadcast_shapes(z_con.shape, embeddings.shape))]
                img_feat_flat = con_feat.img_feat_patch
            
        elif self.args.dif_model_version == 5:
            # voxelization
            grid_resolution = self.args.voxel_resolution
            grid_size = [grid_resolution,grid_resolution,grid_resolution//3]
            x = np.linspace(-0.45, 0.45, grid_size[0])
            y = np.linspace(-0.45, 0.45, grid_size[1])
            z = np.linspace(-0.1, 0.2, grid_size[2])
            xv, yv, zv = np.meshgrid(x, y, z)
            grid = np.stack([xv, yv, zv]).astype(np.float32).reshape(3, -1).transpose() # (np, 3)
            grid = jnp.array(grid)
            grid = grid[None,None] # (1 1 NP 3)

            selected_img_fts, cam_fts = extract_pixel_features(grid, con_feat) # (NB 1 NP NC d)
            ...
            z_con = Aggregator(self.args)(z_con) # (B 1 NG d)
            
            z_con = z_con.reshape((z_con.shape[0], *grid_size, z_con.shape[-1])) # (NB NG1 NG2 NG3 d)
            for _ in range(3):
                z_con = nn.Conv(self.args.base_dim, kernel_size=(3,3,3))(z_con)
                z_con = nn.relu(z_con)

            z_con = z_con.reshape((z_con.shape[0], -1, z_con.shape[-1])) # (NB NP d)
            img_feat_flat = z_con
            context = embeddings
        ...
        if self.args.dif_model_version == 5 and self.args.use_p:
            z = jnp.c_[z, cen_emb]
        z = AdaLayerNorm()(nn.Dense(self.args.base_dim)(z), context)
        
        # apply transformer Decoders
        for _ in range(self.args.first_mixing_depth):
        ...
        if self.args.second_mixing_depth != 0:
            pos_emb2 = nn.Dense(self.args.base_dim//2)(jax.lax.stop_gradient(pos_scaled))
            pos_emb2 = jnp.sin(pos_emb2)
            pos_emb2 = nn.Dense(self.args.base_dim//2)(pos_emb2)

            if self.args.use_p:
                cen_emb2 = nn.Dense(self.args.base_dim//2)(jax.lax.stop_gradient(center_scaled))
                cen_emb2 = jnp.sin(cen_emb2)
                cen_emb2 = nn.Dense(self.args.base_dim//2)(cen_emb2)

            if self.args.dif_model_version in [1,4,6]:
                if self.args.use_p:
                    selected_img_fts2, _ = extract_pixel_features(jax.lax.stop_gradient(center_scaled), con_feat)
                else:
                    selected_img_fts2, _ = extract_pixel_features(jax.lax.stop_gradient(pos_scaled)[...,None,:], con_feat)
                if self.args.spatial_pe:
                    z_con2 = selected_img_fts2 # (B NO ND NC d)
                else:
                    z_con2 = jnp.c_[selected_img_fts2, cam_fts]
                z_con2 = Aggregator(self.args)(z_con2)

                img_fts = z_con2
                if batch_condition_mask is not None:
                    img_fts = jnp.where(batch_condition_mask[...,None,None,None], img_fts, 0)

                if self.args.use_p:
                    context = jnp.c_[cen_emb2, img_fts, jnp.broadcast_to(embeddings, jnp.broadcast_shapes(img_fts[...,:1].shape, embeddings.shape))]
                else:
                    context = jnp.c_[pos_emb2[...,None,:], img_fts, jnp.broadcast_to(embeddings, jnp.broadcast_shapes(img_fts[...,:1].shape, embeddings.shape))]
                z_sh = jnp.c_[z_sh, einops.repeat(pos_emb2, '... d -> ... r2 d', r2=ncd)]
            elif self.args.dif_model_version == 5:
                if self.args.use_p:
                    z_sh = jnp.c_[z_sh, cen_emb2, einops.repeat(pos_emb2, '... d -> ... r2 d', r2=ncd)]
                else:
                    z_sh = jnp.c_[z_sh, einops.repeat(pos_emb2, '... d -> ... r2 d', r2=ncd)]
            z_sh = nn.Dense(self.args.base_dim)(z_sh)
            z_sh = AdaLayerNorm()(z_sh, context)
            for _ in range(self.args.second_mixing_depth):
                z_sh = TransformerDecoder(self.args, dropout, local_only=True)(z_sh, context, img_feat_flat, train=train)
        else:
            z_sh = nn.Dense(self.args.base_dim)(z_sh)
            z_sh = AdaLayerNorm()(z_sh, context)
            # final linear
            z_sh2 = Aggregator(self.args, depth=2, axis=-2, pooled_out=False, pooling_func=jnp.mean, final_activation=False)(z_sh)
            z_sh = z_sh + nn.Dropout(dropout)(z_sh2, deterministic=not train)
            z_sh = AdaLayerNorm()(z_sh, context)
...
class ObservationModel(nn.Module):
    ...
    def __call__(self, objects_ptcl:cxutil.LatentObjects, con_feat:structs.ImgFeatures, time, train=False):
        ...
        # skip = nn.relu(nn.Dense(self.args.base_dim)(z_con))
        # for i in range(3):
        #     z_con = nn.Dense(self.args.base_dim)(z_con)
        #     z_con = nn.relu(z_con)
        #     if i==1:
        #         z_con += skip
        # z_con = jnp.max(z_con, axis=-2) # (B NO ND d)

        # z_con = z_con + nn.SelfAttention(num_heads=4, qkv_features=2*self.args.base_dim, out_features=self.args.base_dim, 
        #                                  dropout_rate=dropout, broadcast_dropout=False)(z_con, deterministic=not train)
